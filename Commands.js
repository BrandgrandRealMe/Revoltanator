...
class CommandBuilder extends EventEmitter {
  ...
  setTranslation(key, property) {
    if (typeof key === "string")
      return (this.translations[property] = { key: key });
...
class CommandRequirement {
  ...
  getPermissions() {
    return this.ownerOnly
      ? [...this.permissions, "Owner-only command"]
      : this.permissions;
  }
...
class CommandHandler extends EventEmitter {
    ...
    if (args[0] === this.acceptCommand && this.fixMap.has(msg.authorId)) {
      ...
      if (!args[1])
        return this.paginateHelp
          ? this.genHelp(null, msg, true)
          : this.replyHandler(
              this.f(
                this.getHelpPage(this.commandLimit, 0, msg, ...this.commands),
      ...
      if (args.length > 2) {
          ...
          return this.replyHandler(
            this.f(
              this.t(
                ...
                "cmdHandler.command.invalid",
                msg,
                { command: "`$prefix" + args[1] + "`" },
              ),
              msg.channel.serverId,
            ),
            msg,
        ...
        return this.replyHandler(
          this.genCommandHelp(this.commands[idx], msg),
          msg,
        );
      }
    }
      ...
      this.replyHandler(
        this.f(
          this.t(
            ...
            "cmdHandler.command.suggestion",
            msg,
            {
              command: "`$prefix" + fixed + "`",
              acceptCommand: "`$prefix$accept`",
            },
          ),
          msg.channel.serverId,
        ),
  ...
  processCommand(cmd, args, msg, previous = false) {
    ...
    for (let i = 0, argIndex = 1; i < options.length; i++) {
      // argIndex starts at 1 to exclude command itself
      ...
      if (!valid && (o.required || !o.empty(value))) {
        // TODO: improve checking on optional options
        let e = o.typeError
    ...
    options
      .filter((o) => !usedOptions.includes(o.uid))
      .forEach((o) => {
  ...
  getDescription(obj, message) {
    ...
    const translated = this.translationHandler(
      config.key,
      message,
      config.options,
    );
  ...
  genHelp(page = null, message, paginate = false, ...cmds) {
    ...
    if (this.paginateHelp && message && paginate) {
      ...
      const contents = cmds.map((cmd, i) => {
        return (
          i + 1 + ". **" + cmd.name + "**: " + this.getDescription(cmd, message)
        );
      });
    ...
    cmds.forEach((cmd, i) => {
      content +=
        ...
        cmd.name +
        "**: " +
        this.getDescription(cmd, message) +
        "\n";
    });
  ...
  genCommandHelp(cmd, msg) {
    ...
    if (cmd.examples.length > 0)
      content +=
        "Example(s): \n- `" +
        cmd.examples
          .map((e) => this.f(e, msg?.channel?.serverId))
          .join("`\n- `") +
        "`\n\n";
    ...
    if (cmd.subcommands.length > 0) {
      ...
      cmd.subcommands.forEach((s) => {
        content +=
          ...
          s.name +
          ": " +
          this.getDescription(s, msg) +
          (s.options.length > 0
            ? "; (`" + s.options.length + " option(s)`)"
            : "") +
          "\n";
      });
      content += "\n";
