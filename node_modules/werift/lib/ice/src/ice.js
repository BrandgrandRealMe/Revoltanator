...
const log = debug_1.default("werift-ice : packages/ice/src/ice.ts : log");
class Connection {
    constructor(iceControlling, options) {
        this.iceControlling = iceControlling;
        this.localUserName = helper_1.randomString(4);
        this.localPassword = helper_1.randomString(22);
        this.remotePassword = "";
        this.remoteUsername = "";
    ...
    async getComponentCandidates(component, addresses, timeout = 5, cb) {
        ...
        for (const address of addresses) {
            ...
            await protocol.connectionMade(net_1.isIPv4(address), this.options.portRange);
            ...
            try {
                const srflxCandidates = (await Promise.all(this.protocols.map((protocol) => new Promise(async (r, f) => {
                    ...
                    if (protocol.localCandidate?.host &&
                        net_1.isIPv4(protocol.localCandidate?.host)) {
        ...
        if (this.turnServer &&
            ...
            const protocol = await protocol_2.createTurnEndpoint(this.turnServer, this.options.turnUsername, this.options.turnPassword, { portRange: this.options.portRange });
            this.protocols.push(protocol);
    ...
    async connect() {
        ...
        // # wait for completion
        const res = this.checkList.length > 0 ? await this.checkListState.get() : ICE_FAILED;
        ...
        this.queryConsentHandle = helper_1.future(this.queryConsent());
        this.setState("connected");
    }
    unfreezeInitial() {
    ...
    schedulingChecks() {
        // Ordinary Check
        {
            ...
            if (pair) {
                pair.handle = helper_1.future(this.checkStart(pair));
                return true;
            }
        }
        {
    ...
    pruneComponents() {
        ...
        const missingComponents = [...helper_1.difference(this._components, seenComponents)];
    ...
    checkIncoming(message, addr, protocol) {
        ...
        if (!remoteCandidate) {
            ...
            remoteCandidate = new candidate_1.Candidate(helper_1.randomString(10), component, "udp", message.attributes["PRIORITY"], host, port, "prflx");
            this.remoteCandidates.push(remoteCandidate);
        }
        // find pair
